...
std::tuple<int, int, QString, int, int, int> MainWindow::calculaStats(Peca a)
{
    ...
    for(auto i : a.roupas){
        if(i.getTipo() == "Camisa" or
            i.getTipo() == "Camiseta" or
            i.getTipo() == "Blusa"
           ){
            t_tronco++;
        }

        if(i.getTipo() == "Saia" or
           i.getTipo() == "Jeans" or
           i.getTipo() == "Bermuda" or
           i.getTipo() == "Shortinho"
          ){
            t_pernas++;
        }

        if(i.getTipo() == "Tenis" or
           i.getTipo() == "Sapatenis" or
           i.getTipo() == "Chinelo"
           ){
            t_calcados++;
        }
...
void MainWindow::on_actionApagar_Tudo_triggered()
{
    ...
    if(QMessageBox::Yes == txt){
        bool i;
        QString resposta = QInputDialog::getText(this, "42", question[idioma], QLineEdit::Normal, "", &i);
        if(resposta.toInt() == 5){
            roupas.roupas.clear();
            ui -> tabela -> clearContents();
            ui -> tabelaStats -> clearContents();
...
void MainWindow::on_apagar_activated(const QString &arg1)
{
